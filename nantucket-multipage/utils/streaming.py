...
from .util import xywh2xyxy, NAMES, colors
...
def check_streaming_url(youtube_link, url, session_state):
    if url is not None:
        try:
            ...
            session_state.stream_url = stream_url
            # return stream_url
    else:
        stream_url = get_streaming_url(youtube_link)
        session_state.stream_url = stream_url
...
# @st.cache_resource
def process_frames(live_url, host, port, frame_placeholder, stop, overlay_on, overlay_off, session_state, selected_model):
    global aggregate_frame_df
                ...
                if overlay_on:
                    ...
                    if len(inference.result.outputs.keys()):
                        ...
                        if len(preds):
                            ...
                            frame_placeholder.image(image=video_frame, caption="Lower Main Street, Nantucket, MA")
                            frame_stats = [
                                [
                                    frame_read_start, 
                                    det["class"],
                            ...
                            frame_placeholder.image(image=frame, caption="Lower Main Street, Nantucket, MA")                             
                            frame_stats = [
                                [
                                    frame_read_start,
                                    "No overlay",
